...
public class UIHandler : MonoBehaviour {
  protected string MyStorageBucket = "gs://savas-delisi.appspot.com";
  private const int kMaxLogSize = 16382;
  protected static string UriFileScheme = Uri.UriSchemeFile + "://";
    public int surum_versiyon;
    public GameObject guncelleme_paneli;
  ...
  protected string storageLocation= "gs://savas-delisi.appspot.com/surum.txt";
  // String to upload to storageLocation or the contents of the file downloaded from
  // storageLocation.
  protected string fileContents="0";
  ...
  protected string localFilename = "surum.txt";
  ...
  protected virtual void Start() {
    ...
    public void initcagir()
    {
        InitializeFirebase1();
    }
  protected virtual void InitializeFirebase1() {
  ...
  protected IEnumerator DownloadToFile1() {
    var storageReference = GetStorageReference();
    var localFilenameUriString = PathToPersistentDataPathUriString(localFilename);
    ...
    if (!(task.IsFaulted || task.IsCanceled)) {
      ...
      fileContents = File.ReadAllText(filename);
            ...
            if(fileContents!="")
            {
                denek.text = fileContents;
                if (surum_versiyon< Convert.ToInt32(denek.text))//PlayerPrefs.GetInt(denek.text))
                {
                   
                  guncelleme_paneli.SetActive(true);
                }
                else if(surum_versiyon == Convert.ToInt32(denek.text))
                {
                    
                    guncelleme_paneli.SetActive(false);
                }
            }
    }
  }
    public void surumcontrol()
    {
        StartCoroutine(DownloadToFile1());
    }
    public void guncelle()
    {
        
        Application.OpenURL("https://play.google.com/store/apps/details?id=com.muratunlu0.savasdelisi");
